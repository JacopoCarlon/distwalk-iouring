...
void *thread_receiver(void *data) {
    ...
    while ((pkt_i = num_success + num_error + num_timedout) < num_pkts) {
        ...
        if (conn->req_list){
            dw_log("client__ thread_receiver: --- currently, conn.req_list is not null\n");
        }else{
            dw_log("client__ thread_receiver: --- currently, conn.req_list is NULL\n");
        }
...
static error_t argp_client_parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    case STORE_DATA: {
        ...
        do {
            if (strncmp(arg, "nosync", 6) == 0) {
                ...
                char *tok = strsep(&arg, ",");
                check(pd_parse_bytes(&val, tok), "Wrong store data size specification");
            }
            if (arg != NULL && *arg == ',')
                arg++;
    ...
    case REPLY_CMD: {
        ...
        // Clamp limits on response size
        val.min = (val.min < MIN_REPLY_SIZE)? MIN_REPLY_SIZE : val.min;
        val.max = (val.max > BUF_SIZE)? BUF_SIZE : val.max;

        if (queue_size(ccmd) <= 0) { // edge-case
            ccmd_add(ccmd, REPLY, &val);
            ccmd_last(ccmd)->resp.mode = reply_mode;
            ccmd_last(ccmd)->resp.dev_id = dev_id;
            break;
        }
